...
// (2) Sampling Categorical
int categorical(const double p[], const int Q, gsl_rng * r) {
	double x = gsl_rng_uniform(r);
	double p_acc[Q-1];
	p_acc[0] = p[0];
	for (int q = 1; q < Q - 1; q++) {
		p_acc[q] = p_acc[q - 1] + p[q];
	}
	int z = 0;
	for (int q = 0; q < Q - 1; q++) {
		if (x > p[q]) z++;
...
// (3) Sampling Dirichlet Distributions (gsl_ran_dirichlet)
...
// (5) vector -> gsl_vector or gsl_matrix
...

{
	...
	// K's
	K_r_y = K_r_y + Q; // no constant, no j-vars, augmenting types
	K_r_n = K_r_n + Q; // no constant, no j-vars, augmenting types
	K_s_y = K_s_y + 2;
	K_s_n = K_s_n + 2;
	...
	map<int, double> s;
	map<int, double> v;	
	map<int, double> z; // new
	map<int, double> p; // new	
	...
	for (int j = 0; j < J; j++) {
		...
		for (int t = born[j] + 1; t <= dead[j]; t++) {
			...
			z[t * J + j] = categorical(tempp, Q, gsl_r);
	...
	for (int g = G1; g < G2; g++) {
			...
			for (int t = born[j]; t <= dead[j]; t++) {
				...
				if (t < dead[j]) {
					...
					if (i == -1) {
						...
						gsl_vector_set(tempv, z[(t + 1) * J + j], 1.0); // z[t * J + j] = q
						gsl_vector_from_to(tempv, indep_r[(t + 1) * J + j], Q, K_r_n);
					...
					// previously matched
						...
						gsl_vector_set(tempv, z[(t + 1) * J + j], 1.0); // z[t * J + j] = q
						gsl_vector_from_to(tempv, indep_r[(t + 1) * J + j], Q, K_r_y);
		...
		// (1) (phi_r_y, sigma2_r_y) (new)
		cout << "update phi_r_y" << endl;
		if (count_y > 0) {
			// initialize
			...
			for (int t = 1; t < T; t++) {
				...
				if (size > 0) {
					...
					for (unsigned int c = 0; c < size; c++) {
						...
						gsl_matrix_set(regX_r_y, c, z[t * J + j], 1.0); // z[t * J + j] = q
						gsl_matrix_from_to(regX_r_y, indep_r[t * J + j], c, Q, K_r_y);						
					}
					#pragma omp critical(dataupdate)
			...
			for (int t = 1; t < T; t++) {
				...
				if (size > 0) {
					...
					for (unsigned int c = 0; c < size; c++) {
						...
						gsl_matrix_set(regX_r_n, c, z[t * J + j], 1.0); // z[t * J + j] = q
						gsl_matrix_from_to(regX_r_n, indep_r[t * J + j], c, Q, K_r_n);				
					}
					#pragma omp critical(dataupdate)
		...
		for (int j = 0; j < J; j++) {
			for (int t = born[j] + 1; t <= dead[j]; t++) {
				...
				if (i == -1) {
					gsl_vector * tempv = gsl_vector_calloc(K_r_n);
					gsl_vector_from_to(tempv, indep_r[t * J + j], Q, K_r_n);
					gsl_blas_ddot(tempv, phi_r_n, &res);
					gsl_vector_free(tempv);
					res = r[(t - 1) * J + j] - r[t * J + j] - res;
					sigma2 = sigma2_r_n;
					for (int q = 0; q < Q; q++) {
						mu[q] = gsl_vector_get(phi_r_n, q);
					}
				}
				else {
					gsl_vector * tempv = gsl_vector_calloc(K_r_y);
					gsl_vector_from_to(tempv, indep_r[t * J + j], Q, K_r_y);
                    gsl_blas_ddot(tempv, phi_r_y, &res);
                    gsl_vector_free(tempv);
                    res = r[(t - 1) * J + j] - r[t * J + j] - res;
                    sigma2 = sigma2_r_y;
					for (int q = 0; q < Q; q++) {
						mu[q] = gsl_vector_get(phi_r_y, q);
					}
				}
				// noramlization
				double tempp[Q];
				double sum = 0;
				for (int q = 0; q < Q; q++) {
					tempp[q] = p[j * Q + q] * exp (-0.5 / sigma2 * (res - mu[q]) * (res - mu[q]));
					sum = sum + tempp[q];
				}
				for (int q = 0; q < Q; q++) {
					tempp[q] = tempp[q] / sum;
		...
		for (int j = 0; j < J; j++) {
			...
			for (int t = born[j] + 1; t <= dead[j]; t++) {
				for (int q = 0; q < Q; q++) {
					if (z[t * J + j]==q) new_n[q] += 1;
				}
			}
			// update p
			double tempp[Q];
			gsl_ran_dirichlet (gsl_r, Q, new_n, tempp);
			for (int q = 0; q < Q; q++) {
				p[j * Q + q] = tempp[q];
			}
		}
				
		// ------------ Store updates in save ------------ //
	...
	out.open("imputed_z.txt");
	out << "j, t, z" << endl;
	for (int j = 0; j < J; j++) {
		for (int t = born[j]+1; t<=dead[j]; t++) {
			out << j << ", " << t << ", " << z[t * J + j] << endl;
		}
	}
	out.close();
	
	out.open("imputed_p.txt");
	out << "j";
	for (int q = 0; q < Q; q++) {
		out << ", p" << q;
	}
